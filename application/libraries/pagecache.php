...
class Pagecache {
    ...
    public function get_partial($key) {
        // Convenience function
        return $this->get($key, TRUE);
    }

    public function set_partial($key, $data = "") {
        // Convenience function
        return $this->set($key, $data, TRUE);
    }

    public function get($key, $partial = FALSE) {
        if (!$this->enabled and !$partial) {
            return FALSE;
        }
    ...
    public function set($key, $data = "", $partial = FALSE) {
        if (!$this->enabled and !$partial) {
            return FALSE;
        }
        ...
        if ($this->cache_hit <= CACHE_HIT_NONE) {
            if (!$partial) {
                $data = "$data<!-- disk {$this->timestamp} -->";
            }

            file_put_contents("{$this->APPPATH}cache/$hashed_key.html", $data);
        }
